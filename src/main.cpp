      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
					...
					double Dt = double(std::clock() - last_time) / CLOCKS_PER_SEC;
					last_time = std::clock();

					double diff_cte = (cte - pid.cte) / Dt;
					pid.UpdateError(cte);
					double int_cte = pid.TotalError();

					double steer_value = -(pid.Kp * cte + pid.Kd * diff_cte + pid.Ki * int_cte);
